...
def forecast_wealth(
    ...
    invested = s_invested
    pension = s_pension
    real_estate_equity = s_real_estate_equity

    for y in range(0, years + 1):
        nw = liquid + invested + pension + real_estate_equity
        ...
        rows.append({
            ...
            "net_worth": nw,
            "net_worth_real": nw * rf
        })

        if y < years:
            invested *= (1 + exp_ret / 100)
            pension *= (1 + exp_ret / 100)
            real_estate_equity *= (1 + real_estate_growth / 100)
...
with st.sidebar:
    ...
    investments = st.number_input("Stock and Shares (£)", 0, 50_000_000, int(existing.get("investments", 85_000)), 1_000)
    pension_val = st.number_input("Pension (£)", 0, 50_000_000, int(existing.get("pension", 42_000)), 1_000)

    real_estate_equity_default = existing.get(
        "real_estate_equity",
        existing.get("house_equity", max(0, existing.get("property", 350_000) - existing.get("mortgage", 220_000)))
    )
    real_estate_equity = st.number_input("Real Estate Equity Value (£)", 0, 50_000_000, int(real_estate_equity_default), 5_000)

    st.markdown(f"""<div style="background:linear-gradient(135deg,{CARD} 0%,{CARD_H} 100%);border:1px solid {BORDER};border-radius:12px;padding:.8rem 1rem;margin:.7rem 0 .6rem 0;text-align:center;">
    ...
    st.markdown(sidebar_label("Monthly Money Inputs", BLUE), unsafe_allow_html=True)
    ...
    if st.button("Save Snapshot", use_container_width=True):
        nw = cash + investments + pension_val + real_estate_equity
        sd = {
            "cash": cash,
            ...
            "scotland_tax": scotland_tax,
            "pension_contrib_pct": pension_contrib_pct,
    ...
    if st.session_state.snapshot_saved_flag:
        st.markdown(
            f'<div style="background:{GREEN}18;border:1px solid {GREEN}44;border-radius:8px;padding:.5rem .7rem;text-align:center;margin-top:.3rem;"><span style="color:{GREEN};font-size:.8rem;font-weight:600;">Snapshot saved for {period_label}</span></div>',
            unsafe_allow_html=True
        )
        st.session_state.snapshot_saved_flag = False
...
def build_history_df(snaps):
    ...
    for pk, sd in sorted(snaps.items()):
        ...
        re_equity = sd.get(
            "real_estate_equity",
            sd.get("house_equity", max(0, sd.get("property", 0) - sd.get("mortgage", 0)))
        )
        nw = sd.get("net_worth", sd.get("cash", 0) + sd.get("investments", 0) + sd.get("pension", 0) + re_equity)
...
net_worth = cash + investments + pension_val + real_estate_equity
...
<div style="color:{TEXT2};font-size:.8rem;">Fill in: <b>Cash Savings</b>, <b>Stock and Shares</b>, <b>Pension</b>, and <b>Real Estate Equity Value</b>.</div></div></div>
...
with tab_overview:
    ...
    c5.markdown(kpi(LBL_RE, gbp(real_estate_equity), AMBER, "⬡"), unsafe_allow_html=True)
    c6.markdown(kpi("Monthly Surplus", gbp(surplus), CYAN if surplus >= 0 else RED, "↗" if surplus >= 0 else "↘"), unsafe_allow_html=True)
    c7.markdown(kpi("Savings Rate", pct_fmt(savings_rate), PURPLE, "◉"), unsafe_allow_html=True)
    c8.markdown(kpi("10-Year Forecast", gbp(forecast_10y_val), BLUE, "⟩"), unsafe_allow_html=True)
    ...
    with col_d:
        ...
        labels = [LBL_CASH, LBL_STOCK, LBL_PENSION, LBL_RE]
        values = [cash, investments, pension_val, real_estate_equity]
    ...
    with col_b:
        ...
        cats = [LBL_CASH, LBL_STOCK, LBL_PENSION, LBL_RE]
        vals = [cash, investments, pension_val, real_estate_equity]
        clrs = [CYAN, BLUE, GREEN, AMBER]
...
with tab_history:
    ...
    if history_df.empty:
        ...
        for col_name, lbl, clr in [
            ...
            ("real_estate_equity", LBL_RE, AMBER),
        ]:
...
with tab_portfolio:
    ...
    with cl:
        ...
        fig = go.Figure(go.Pie(
            labels=[LBL_STOCK, LBL_PENSION],
            values=[investments, pension_val],
            hole=0.6,
            marker=dict(colors=[BLUE, GREEN], line=dict(color=BG, width=2)),
    ...
    with cr:
        ...
        fig = go.Figure(go.Bar(
            x=["Investment", "Pension (you)", "Pension (employer)"],
            y=contrib_vals,
            marker=dict(color=[BLUE, GREEN, CYAN]),
...
with tab_forecast:
    ...
    for df_sc, clr in [(df_con, CLR_CON), (df_base, CLR_BASE), (df_agg, CLR_AGG)]:
        last_row = df_sc.iloc[-1]
        fig.add_annotation(
            x=last_row["year"], y=last_row["net_worth"],
    ...
    with cl:
        ...
        for clr, col in [(BLUE, "net_worth"), (CYAN, "net_worth_real")]:
            last_v = df_base.iloc[-1][col]
            fig2.add_annotation(
                x=df_base.iloc[-1]["year"], y=last_v,
    ...
    with cr:
        ...
        fig3 = go.Figure(go.Bar(
            x=["Conservative", "Base", "Aggressive"],
            y=rv,
            marker=dict(color=[CLR_CON, CLR_BASE, CLR_AGG]),
            text=[gbp(v) for v in rv],
            textposition="inside",
            textfont=BOLD_WHITE,
...
with tab_goals:
    ...
    fig_g.add_annotation(
        x=df_base.iloc[-1]["year"], y=last_nw,
    ...
    if len(crossing) > 0:
        ...
        fig_g.add_vline(
            x=cy, line_dash="dot", line_color=GREEN, line_width=1.5,
    ...
    fig_g.update_layout(**make_layout({
        "height": 360,
        "showlegend": False,
        "xaxis": {**GRID_AXIS, "title": {"text": "Years"}},
        "yaxis": GRID_AXIS,
    }))
...
with tab_assumptions:
    ...
    for lb, vl, desc, clr in [
        ...
        ("Property Growth Rate", pct_fmt(property_growth), "Annual real estate equity growth assumption", AMBER),
...
with tab_salary:
    ...
    <span style="color:{TEXT3};font-size:.75rem;">{"Scotland" if scotland_tax else "England / Wales / NI"} · current rates</span></div>""", unsafe_allow_html=True)
